...
sasnapshot = subprocess.check_output('sudo zfs snapshot mypool/ftp@sademo_'+CURRENT , shell=True, stderr=subprocess.STDOUT)
...
if len(listcheck.split('\n')) > 9:
    ...
    byebye = subprocess.check_output('sudo zfs destroy '+deletesnap , shell=True, stderr=subprocess.STDOUT)
